...
public class BizEventAdviceProcessEnter implements BizEventAdviceChain {
    ...
    @Override
    public Object doProcess(EventContext context, Object event, ProceedingJoinPoint joinPoint) throws Throwable {
        Object result = null;
        if (pos < size) {
            BizEventAdvice bizEventAdvice = bizEventAdviceList.get(pos++);
            result = bizEventAdvice.process(context, event, joinPoint, this);
        }
        return result;
    }
